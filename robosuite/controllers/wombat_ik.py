...
class InverseKinematicsController(Controller):
    """
    ...
    def inverse_kinematics(self, target):
        ...
        """
        ee_pose_real = invK.sim2real_wrapper(target)
        joint_real = trg.jointTraj(ee_pose_real)
        joint_sim = invK.ik_wrapper(joint_real)
        global PD_scale
        PD_scale = PD_signal_scale(ee_pose_real,joint_real)
        # ik_solution = list(
    ...
    def set_goal(self, action, set_qpos=None):
        ...
        print("3")
        set_qpos = inverse_kinematics(action)
    ...
    def run_controller(self):
        ...
        self.kp = np.array([150,150,150,20*PD_scale,20*PD_scale,20*PD_scale])
        self.kd = np.array([1500,1500,1500,0.6,0.6,0.6])
        ...
        desired_torque = (np.multiply(np.array(position_error), self.kp)
